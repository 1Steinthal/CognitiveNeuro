# -*- coding: utf-8 -*-
""" DESCRIPTION:
This fMRI/MEG/EEG experiment displays 3 different types of words (positive, negative,neutral),
...
NOTES FROM LAURA: Nice nemonic for the keys to press
    y = whYYYY so negative
    b = Bositive <333 so positive
"""
...
V= {'ID':'','session':[1,2,3,4,5,6],'Scan day': ['Tuesday','Wednesday','Friday'],'gender':['male','female'],'age':''} # experiment type was not used further along in the script so I removed it
if not gui.DlgFromDict(V, order=['ID','Scan day', 'age', 'session','gender']).OK: # dialog box; order is a list of keys
    core.quit()
...
## STIMULI ##
# image files
IMG_P = 'image_stim_p.png' # positive
IMG_N = 'image_stim_n.png' # negative
...
# Factorial design
    ...
    for word in range(words.shape[0]):
        ...
        if words.label[word]=='pos': # if word is positive then image is positive
            img = IMG_P #image file
            TRIG_I = 21 #trigger code
            TRIG_W = 11
            TRIG_BEFORE = 31

        if words.label[word]=='neg': # if word is negative then image is negative
        ...
        if words.label[word]=='neu':
            img = sample([IMG_P,IMG_N],1)[0] # randomly select image if word is neutral
            TRIG_W = 13
            if img==IMG_P:
                TRIG_I = 41
...
#                    if trial['word_label']=='neu': # neutral word before
                        if trial['img']==IMG_N:
                            ...
                            win.callOnFlip(setParallelData, 112)  # pull trigger up; 100 = correct; XX2 neg-response
                            pullTriggerDown = True
                        elif trial['img']==IMG_P:
                            trial['correct_resp'] = 0
                            win.callOnFlip(setParallelData, 212)  # pull trigger up; 200 = incorrect; XX2 neg-response
                            pullTriggerDown = True
                    else: # positive or negative word before
                        if trial['img']==IMG_N:
                            trial['correct_resp'] = 1
                            win.callOnFlip(setParallelData, 102)  # pull trigger up; 100 = correct; XX2 neg-response
                            pullTriggerDown = True
                        elif trial['img']==IMG_P:
                            trial['correct_resp'] = 0
                            win.callOnFlip(setParallelData, 202)  # pull trigger up; 200 = incorrect; XX2 neg-response
                            pullTriggerDown = True
                    ...
                    if trial['word_trigger']==13:
                        if trial['img']==IMG_P:
                            trial['correct_resp'] = 1
                            win.callOnFlip(setParallelData, 111)  # pull trigger up; 100 = correct; XX1 pos-response
                            pullTriggerDown = True
                        ...
                        if trial['img']==IMG_N:
                            trial['correct_resp'] = 1
                            win.callOnFlip(setParallelData, 101)  # pull trigger up; 100 = correct; XX1 pos-response
                            pullTriggerDown = True
                        elif trial['img']==IMG_P:
                            trial['correct_resp'] = 0
                            win.callOnFlip(setParallelData, 201)  # pull trigger up; 200 = incorrect; XX1 pos-response
                            pullTriggerDown = True
